...
@Service
public class PdfSigningService_20251015_1500 {
    ...
    /**
     * Enhanced method to extract signer name from certificate with fallback options
     */
    private String extractSignerName(PdfPKCS7 pkcs7) {
        try {
            // Try to get signer name from the certificate subject
            java.security.cert.X509Certificate cert = pkcs7.getSigningCertificate();
            if (cert != null) {
                ...
                // If no CN, try Organization (O)
                String org = extractOrganization(subject);
                if (org != null && !org.trim().isEmpty()) {
                    return org;
                }

                // Return the full subject if no better option
                return subject;
            }
        } catch (Exception e) {
            System.err.println("Error extracting signer name: " + e.getMessage());
        }

        // Fallback to original method
        String signName = pkcs7.getSignName();
        return (signName != null && !signName.trim().isEmpty()) ? signName : "Unknown Signer";
    }
    ...
    private String extractOrganization(String subject) {
        ...
        // Look for O= in the subject string
        String[] parts = subject.split(",");
        for (String part : parts) {
            if (part.trim().toUpperCase().startsWith("O=")) {
                return part.trim().substring(2).trim();
            }
        }
        return null;
    }
